...
MES_ROC_results = matrix(ncol=7, nrow=length(seq(floor(min(merged_MES_data$WT)), ceiling(max(merged_MES_data$WT)))))
i = 1
for(cutoff in seq(floor(min(merged_MES_data$WT)), ceiling(max(merged_MES_data$WT))))
{
  ...
  tp = (length(merged_MES_data$WT[merged_MES_data$WT > cutoff & merged_MES_data$src=="REFSEQ"]))/1000
  # false negatives include all refseq sites not called (i.e. those with scores == 990)
  fn = (refseq_MES_uncalled + length(merged_MES_data$WT[merged_MES_data$WT < cutoff & merged_MES_data$src=="REFSEQ"]))/1000
  # true negatives include all random sites not called
  tn = (random_MES_uncalled + length(merged_MES_data$WT[merged_MES_data$WT < cutoff & merged_MES_data$src=="RANDOM"]))/1000
  fp = length(merged_MES_data$WT[merged_MES_data$WT > cutoff & merged_MES_data$src=="RANDOM"])/1000
